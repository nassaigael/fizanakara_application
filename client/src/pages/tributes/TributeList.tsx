...
const TributeList: React.FC = () => {
  const navigate = useNavigate();
  const { tributes, loadingTributes, deleteTribute } = useTribute();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success('Tribu supprimée avec succès');
      setDeleteId(null);
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6 p-4 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase">
              {filteredTributes.length} résultat(s)
            </p>
          </div>
        </div>
        <Button
          onClick={() => navigate('/admin/tributes/create')}
          className="flex items-center gap-2"
        >
          <AiOutlinePlus /> NOUVELLE TRIBU
        </Button>
      </div>
            ...
            <tbody className="divide-y-2 divide-gray-50">
              {loadingTributes ? (
                ...
                filteredTributes.map((tribute) => (
                  <tr key={tribute.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="p-6">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-2xl bg-gray-100 flex items-center justify-center font-black text-xs uppercase">
                          <AiOutlineTeam className="text-gray-500" size={18} />
                        </div>
                        <span className="font-black text-sm uppercase">{tribute.name}</span>
                      </div>
                    </td>
                    <td className="p-6">
                      <div className="flex justify-end gap-2">
                        <ActionBtn
                          ...
                          title="Modifier"
                          variant="edit"
                          onClick={() => navigate(`/admin/tributes/${tribute.id}/edit`)}
                        />
                        <ActionBtn
      ...
      <Alert
        ...
        message={`Êtes-vous sûr de vouloir supprimer la tribu "${deleteName}" ? Cette action est irréversible.`}
