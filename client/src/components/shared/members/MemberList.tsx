        ...
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-center">
                    ...
                    <p className="font-black text-gray-500">Chargement des membres...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                    <div className="relative">
                        <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 border-gray-200 flex items-center justify-center bg-gray-100">
                            {(() => {
                                const avatarUrl = getImageUrl(member.imageUrl, `${member.firstName} ${member.lastName}`);
                                if (avatarUrl) {
                                    return (
                                        <img
                                            src={avatarUrl}
                    ...
                    <div>
                        <h3 className="font-black text-lg">{member.firstName} {member.lastName}</h3>
                    ...
                    <tr key={member.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-gray-200 flex items-center justify-center bg-gray-100">
                                    {(() => {
                                        const avatarUrl = getImageUrl(member.imageUrl, `${member.firstName} ${member.lastName}`);
                                        if (avatarUrl) {
                                            return (
                        ...
                        <td className="px-6 py-4">
                            <p className="font-bold text-xs">{member.districtName}</p>
                            <p className="text-[10px] text-gray-500">{member.tributeName}</p>
                        </td>
                        <td className="px-6 py-4">
                            <span className={`text-[8px] font-black uppercase px-2 py-1 rounded-full ${getStatusColor(member.status)}`}>
                                {getMemberStatusLabel(member.status)}
