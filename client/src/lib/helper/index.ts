...
export const getGenderLabel = (gender: Gender | string): string => {
    ...
    return labels[gender] || 'Unspecified';
};

export const getMemberStatusLabel = (status: MemberStatus | string): string => {
    const labels: Record<string, string> = {
        [MemberStatus.STUDENT]: 'Student',
        [MemberStatus.WORKER]: 'Worker',
    };
...
export const getContributionStatusInfo = (status: string) => {
    const statusMap = {
        PENDING: { label: 'Pending', color: 'bg-yellow-100 text-yellow-800', progress: 0 },
        PARTIAL: { label: 'Partial', color: 'bg-blue-100 text-blue-800', progress: 50 },
        PAID: { label: 'Paid', color: 'bg-green-100 text-green-800', progress: 100 },
        OVERDUE: { label: 'Overdue', color: 'bg-red-100 text-red-800', progress: 0 },
    };
